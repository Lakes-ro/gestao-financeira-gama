 ...
 *
 * CORREÇÃO: este ficheiro continha "import { supabase } from
 * './config.js'" e "import {...} from './admin.js'" — sintaxe de
 * ES module dentro de um <script> comum (sem type="module") em
 * admin.html. Isso gera um SyntaxError imediato no navegador e
 * impede TODO o resto do ficheiro de rodar, incluindo initClientes.
 * Resultado prático: a aba "Clientes" (a primeira e ativa por
 * padrão) nunca carregava nada. Reescrito como script global, no
 * mesmo padrão já usado em dashboards.js/metas.js/planejamentos.js/
 * razonete.js.
 */
...
async function renderClientes() {
  ...
  grid.innerHTML = allClientes.map(c => `
    <div class="card">
      ...
      <div class="card-link-row">
        <span class="card-link-text">${window.location.origin}/index.html</span>
        <button class="btn-copy" data-copy="${window.location.origin}/index.html">Copiar</button>
      </div>
  ...
  // Nota: o link acima aponta para a página de login do cliente
  // (index.html). Não existe (ainda) uma página de deep-link por
  // cliente específico — os clientes autenticam-se via Supabase
  // Auth, não por parâmetro de URL.
